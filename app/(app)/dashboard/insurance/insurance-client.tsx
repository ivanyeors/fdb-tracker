...
export type InsuranceInitialData = {
  policies: Policy[]
  coverage: HouseholdCoverageAnalysis | null
}
...
export function InsuranceClient({
  ...
  const coverageUrl = buildUrl(
    "/api/insurance/coverage",
    activeProfileId,
    activeFamilyId
  )

  const { data: policiesData, isLoading: policiesLoading } = useApi<Policy[]>(
    policiesUrl,
    { fallbackData: initialData.policies }
  )
  const { data: coverageRaw, isLoading: coverageLoading } =
    useApi<HouseholdCoverageAnalysis>(coverageUrl, {
      fallbackData: initialData.coverage ?? undefined,
    })

  const policies = useMemo(() => policiesData ?? [], [policiesData])
  const coverageData = coverageRaw ?? null
  const isLoading = policiesLoading || coverageLoading
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {policies.map((policy) => {
                        ...
                        const customBenefits = policy.coverages.filter(
                          (c) => !c.coverage_type
                        )
                              ...
                              <td className="hidden px-4 py-3 text-muted-foreground lg:table-cell">
                                {policy.inception_date
                                  ? new Intl.DateTimeFormat("en-SG", {
                                      month: "short",
                                      year: "numeric",
                                    }).format(new Date(policy.inception_date))
                                  : "—"}
                              </td>
                                  ...
                                  <div className="space-y-1">
                                        ...
                                        >
                                          ...
                                          >
                                            {COVERAGE_TYPE_LABELS[
                                              ...
                                              c.coverage_type!.replaceAll("_", " ")}
                                          </Badge>
                                          <span className="text-xs tabular-nums text-muted-foreground">
                                            {c.coverage_amount > 0
                                              ? `$${formatCurrency(c.coverage_amount)}`
                                              : "—"}
                                          </span>
                                        </div>
                                      ))}
                                    {customBenefits.length > 0 && (
                                      <span className="text-[10px] text-muted-foreground">
                                        +{customBenefits.length} benefit
                                        {customBenefits.length > 1 ? "s" : ""}
                                      </span>
                                    )}
                                  </div>
                                ) : (
                                  <span className="text-muted-foreground">
                                    {policy.coverage_type?.replaceAll("_", " ") ??
                                      "—"}
                                ...
                                {policy.cpf_premium != null &&
                                  policy.cpf_premium > 0 && (
                                    <span className="block text-xs text-blue-600 dark:text-blue-400">
                                      ${formatCurrency(policy.cpf_premium)}/yr
                                      CPF
                                    </span>
                                  )}
                              </td>
                                              ...
                                              .map((c, i) => (
                                                ...
                                                >
                                                  ...
                                                  <td className="py-1.5 pr-4 text-right tabular-nums">
                                                    {c.renewal_bonus != null &&
                                                    c.renewal_bonus > 0
                                                      ? `$${formatCurrency(c.renewal_bonus)}`
                                                      : "—"}
                                                  </td>
                                                  <td className="py-1.5">
                    ...
                    <tfoot>
                      <tr className="border-t bg-muted/50 font-medium">
                        ...
                        <td className="hidden px-4 py-3 text-right tabular-nums lg:table-cell">
                          {(() => {
                            const totalCash = activePolicies.reduce(
                              (s, p) => s + (p.cash_value ?? 0),
                              0
                            )
                            return totalCash > 0
