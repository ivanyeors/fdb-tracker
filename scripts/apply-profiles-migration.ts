#!/usr/bin/env npx tsx
/**
 * Applies the profiles migration to fix "Error Loading Profiles" / "table not found" error.
 *
 ...
 *   npx tsx scripts/apply-profiles-migration.ts
 *
 * Option C - Run SQL manually in Supabase Dashboard → SQL Editor:
 *   Copy contents of supabase/migrations/004_ensure_profiles.sql
 */
...
if (!dbUrl) {
  ...
  console.error("  2. Paste contents of supabase/migrations/004_ensure_profiles.sql")
...
async function main() {
  ...
  const migrationPath = resolve(process.cwd(), "supabase/migrations/004_ensure_profiles.sql")
  ...
  try {
    ...
    console.log("✅ Migration applied successfully. profiles table is ready.")
  } catch (err) {
