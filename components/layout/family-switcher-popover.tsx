...
import { CreateFamilyDialog } from "@/components/layout/create-family-dialog"
...
export function FamilySwitcherPopover() {
  ...
  function handleCreated(family: { id: string; name: string }) {
    setActiveFamilyId(family.id)
    queueMicrotask(() => {
      try {
        document.cookie = `fdb-active-family-id=${family.id}; path=/; max-age=31536000; SameSite=Lax`
      } catch {
        // ignore
      }
    })
